...
void hi() {
    new EventListener<EventFilter<ModPopupUIEvent>>(
        +[](ModPopupUIEvent* event) {
            ...
            if (popup = typeinfo_cast<FLAlertLayer*>(event->getPopup()); popup) void();
            ...
            if (tabsMenu = popup->getChildByIDRecursive("tabs-menu"); tabsMenu) {
                if (tabsMenu->getUserObject("pointAndSizeFixMark"_spr)) void();
                else {
            ...
            if (tabsMenu) void(); else return ListenerResult::Propagate;
            ...
            if (tabsMenu->getChildByID("comments")) void();
                ...
                (CCMenuItemSpriteExtra* sender) {
                    ...
                    if (textarea) void(); else return;
                    auto pFetchTextArea = textarea->getParent();
                    if (pFetchTextArea) void(); else return;
